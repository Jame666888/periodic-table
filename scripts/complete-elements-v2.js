/**
 * 直接修改 elements.js，补全缺失的数据字段（v2）
 * 运行: node complete-elements-v2.js
 */
...
// 方法：将 const 改为 var，然后用 Function 执行
console.log('🔍 解析元素数据...');

// 替换 const 为 var，这样变量会在函数作用域内
let modifiedCode = content.replace('const ELEMENTS = ', 'var ELEMENTS = ');

// 创建函数并执行
try {
  const fn = new Function(modifiedCode + '; return ELEMENTS;');
  const elements = fn();
